...
export class StreetService {
  ...
  private streetsStore: StreetsStore,
  ...
  getStreets(id: string) {
    const page =  this.streetQuery.getValue().currentPage;
    const pageSize = this.streetQuery.getValue().pageSize;

    return this.http.get<StreetWithPaginationModel>(`${this._url}?id=${id}&page=${page}&pageCount=${pageSize}`).pipe(
      tap(entity => {
